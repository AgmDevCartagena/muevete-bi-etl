package com.muevete.bi.etl.service;

import com.muevete.bi.etl.config.AppConfig;
import com.muevete.bi.etl.db.PostgresClient;
import com.muevete.bi.etl.util.RowStore;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.io.File;
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import java.sql.Types;

/**
 * Paso final: TRUNCATE + INSERT de recaudo y cartera en las tablas destino
 * de PostgreSQL, leyendo desde los RowStore generados por Oracle.
 * Toda la carga se hace en una transaccion por tabla.
 */
public class CargaDestinoService {

    private static final Logger log = LoggerFactory.getLogger(CargaDestinoService.class);

    private final AppConfig cfg;
    private final PostgresClient pg;

    public CargaDestinoService(AppConfig cfg, PostgresClient pg) {
        this.cfg = cfg;
        this.pg = pg;
    }

    public void ensureDestinos() throws SQLException {
        try (Connection c = pg.getConnection(); var st = c.createStatement()) {
            st.execute("""
                CREATE TABLE IF NOT EXISTS public."RECAUDO_MULTAS_SAST_TURBACO" (
                    "FUENTE" VARCHAR(50),
                    "TIPO_CARTERA" INTEGER,
                    "FECHA_PAGO" DATE,
                    "RECIBO" VARCHAR(50),
                    "VALOR_RECIBO" NUMERIC(18,2),
                    "TIPO_DOCUMENTO" VARCHAR(50),
                    "IDENTIFICACION" VARCHAR(50),
                    "NOMBRE" VARCHAR(300),
                    "VEHI_PLACA" VARCHAR(20),
                    "COMP_NUMERO" VARCHAR(50),
                    "COMP_FECHA" DATE,
                    "ANIO_COMPARENDO" VARCHAR(4),
                    "PRESCRIPCION" VARCHAR(20),
                    "TIPO_COMPARENDO" VARCHAR(50),
                    "CLASE_VEHICULO" VARCHAR(100),
                    "TIPO" VARCHAR(50),
                    "SERVICIO_VEHICULO" VARCHAR(100),
                    "VALOR_PAGADO" NUMERIC(18,2),
                    "DISTRI_FECHA" DATE,
                    "RESOLUCION_MP" VARCHAR(100),
                    "CART_VALOR_INICIAL" NUMERIC(18,2),
                    "CONCEPTO" VARCHAR(100),
                    "ESTADO_CARTERA" VARCHAR(200),
                    "CONCEPTO_PRINCIPAL" VARCHAR(200),
                    "GESTION" VARCHAR(50),
                    "PORC_DESCUENTO" VARCHAR(20),
                    "DESCUENTO_CARTERA" NUMERIC(18,2),
                    "DESCUENTO_INTERESES" NUMERIC(18,2),
                    "CANT_DESCUENTO_CARTERA" INTEGER,
                    "CANT_DESCUENTO_INTERESES" INTEGER,
                    "SEGUI_RESOLUCION" VARCHAR(100),
                    "SEGUI_FECHA_RESOL" DATE,
                    "VALOR_INTERES" NUMERIC(18,2)
                )
                """);
            st.execute("""
                CREATE TABLE IF NOT EXISTS public."CARTERA_MULTAS_SAST_TURBACO" (
                    "COMP_NUMERO" VARCHAR(20),
                    "COMP_FECHA" TIMESTAMP,
                    "TIPO_COMPARENDO" VARCHAR(7),
                    "CART_VALOR_INICIAL" NUMERIC(20),
                    "COMP_ESTADO" VARCHAR(80),
                    "COMP_CODIGO" NUMERIC(20),
                    "TIPO_INFRACTOR" VARCHAR(80),
                    "PER_TIPO_DOCUMENTO" VARCHAR(80),
                    "PER_NRO_IDENTIFICACION" VARCHAR(20),
                    "PER_NOMBRES" VARCHAR(201),
                    "PER_DIRECCION_INFRACTOR" VARCHAR(80),
                    "PER_EMAIL_INFRACTOR" VARCHAR(50),
                    "PER_TELEFONO_INFRACTOR" VARCHAR(15),
                    "VEHI_PLACA" VARCHAR(10),
                    "INFR_CODIGO_VISISBLE" VARCHAR(5),
                    "PROCESO_ESTADO" VARCHAR(100),
                    "FECHA_ACTUACION" TIMESTAMP,
                    "RESOLUCION_ACTUACION" VARCHAR(80),
                    "FECHA_SANCION" TIMESTAMP,
                    "RESOLUCION_SANCION" VARCHAR(80),
                    "MP_FECHA" TIMESTAMP,
                    "MP_RESOLUCION" VARCHAR(20),
                    "MP_NOTIF_FECHA" TIMESTAMP
                )
                """);
            st.execute("CREATE INDEX IF NOT EXISTS ix_recaudo_compnum ON public.\"RECAUDO_MULTAS_SAST_TURBACO\" (\"COMP_NUMERO\")");
            st.execute("CREATE INDEX IF NOT EXISTS ix_cartera_compnum ON public.\"CARTERA_MULTAS_SAST_TURBACO\" (\"COMP_NUMERO\")");
        }
    }

    public int cargarRecaudo(File store) throws Exception {
        String insert = "INSERT INTO public.\"RECAUDO_MULTAS_SAST_TURBACO\" VALUES (" +
                "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        return cargar(store, cfg.postgres.recaudoTable, insert, RecaudoService.NUM_COLUMNS, "RECAUDO_MULTAS_SAST_TURBACO");
    }

    public int cargarCartera(File store) throws Exception {
        String insert = "INSERT INTO public.\"CARTERA_MULTAS_SAST_TURBACO\" VALUES (" +
                "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        return cargar(store, cfg.postgres.carteraTable, insert, CarteraService.NUM_COLUMNS, "CARTERA_MULTAS_SAST_TURBACO");
    }

    private int cargar(File store, String tablaLog, String insertSql, int expectedCols, String quotedTableName) throws Exception {
        int inserted = 0;
        long t0 = System.currentTimeMillis();
        try (Connection c = pg.getConnection()) {
            c.setAutoCommit(false);
            try {
                if (cfg.etl.truncateBeforeInsert) {
                    try (var st = c.createStatement()) {
                        st.execute("TRUNCATE TABLE public.\"" + quotedTableName + "\"");
                    }
                    log.info("TRUNCATE public.\"{}\" OK", quotedTableName);
                }

                try (PreparedStatement ps = c.prepareStatement(insertSql);
                     RowStore.Reader r = new RowStore.Reader(store)) {

                    Object[] row;
                    int batch = 0;
                    while ((row = r.next()) != null) {
                        if (row.length != expectedCols) {
                            throw new IllegalStateException("Fila con " + row.length +
                                    " columnas, se esperaban " + expectedCols);
                        }
                        for (int i = 0; i < row.length; i++) {
                            if (row[i] == null) ps.setNull(i + 1, Types.OTHER);
                            else ps.setObject(i + 1, row[i]);
                        }
                        ps.addBatch();
                        inserted++;
                        if (++batch % cfg.etl.insertBatchSize == 0) {
                            ps.executeBatch();
                        }
                    }
                    ps.executeBatch();
                }
                c.commit();
            } catch (Exception ex) {
                c.rollback();
                throw ex;
            }
        }
        log.info("Carga {} -> {} filas en {} ms", tablaLog, inserted, System.currentTimeMillis() - t0);
        return inserted;
    }
}
